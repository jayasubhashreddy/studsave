...
router.post('/', auth, async (req, res) => {
  try {
    ...
    res.status(201).json({ ...subject.toObject(), pin: undefined });
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
// Lock: set PIN (stored as plain text)
...
router.post('/:id/verify-pin', auth, async (req, res) => {
  try {
    ...
    if (subject.pin !== String(pin)) return res.status(401).json({ message: 'Wrong PIN' });
...
router.post('/:id/remove-lock', auth, async (req, res) => {
  try {
    ...
    const subject = await Subject.findOne({ _id: req.params.id, userId: req.user._id });
    if (!subject) return res.status(404).json({ message: 'Not found' });
    if (subject.pin && subject.pin !== String(pin)) {
      return res.status(401).json({ message: 'Wrong PIN' });
    }
    const updated = await Subject.findByIdAndUpdate(
      req.params.id,
      { isLocked: false, pin: null },
      { new: true }
    ).select('-pin');
    res.json(updated);
  } catch (err) { res.status(500).json({ message: err.message }); }
